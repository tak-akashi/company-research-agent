...
**受け入れ条件**:
- [x] 企業コード（証券コード/EDINETコード）で書類を検索できる
- [x] 企業名（部分一致）で書類を検索できる
- [x] 期間を指定して検索結果を絞り込める
- [x] 自然言語での期間表現（「過去1年間」「過去半年」等）を理解して検索できる
- [x] 書類種別（有価証券報告書、四半期報告書、臨時報告書）でフィルタリングできる
- [x] PDF/XBRLファイルをダウンロードできる
- [x] APIレート制限に対応したリトライ機構を持つ
...
| 自然言語期間 | 「過去1年間」「過去半年」「2024年1月以降」 | 適切な日付範囲に変換して検索 |
...
**受け入れ条件**:
- [x] 企業のIRページから決算短信・決算説明会資料（PDF）を取得できる
- [x] 中期経営計画のPDFを取得できる
- [x] IRニュースの一覧を取得できる
- [x] robots.txtを遵守し、適切な間隔でアクセスする
- [x] IRテンプレートによる再利用可能なスクレイピング設定
- [x] LLMによる動的なIRページ探索（テンプレートなしでも対応）
- [x] PDF自動ダウンロードと要約生成

**実装内容**:
- `cra ir-fetch`: IR資料の取得・ダウンロード・要約
- `cra ir-template`: スクレイピングテンプレートの管理（作成・検証）
- Playwright + BeautifulSoup による動的ページ対応
- カテゴリ別取得（earnings:決算, news:ニュース, disclosures:適時開示）
